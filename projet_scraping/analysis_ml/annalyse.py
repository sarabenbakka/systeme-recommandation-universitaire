...
path = r"C:\Users\ThinkPad\OneDrive\Desktop\Master Big Data\S1 MASTER\BASE DE DONNEES ORACLE\PROJECTS\projet_scraping\projet_scraping\output\final_jobs_data_cleaned.csv"  # Path to the cleaned CSV
df = pd.read_csv(path)

# 🧹 Nettoyage des données (if necessary, you can repeat this cleaning here, or use the already cleaned data)
df.columns = df.columns.str.strip().str.title()

# Nettoyage de la colonne 'Experience' pour extraire le nombre d'années
...
# ----------------------------------------------------
# 🔮 COLONNE 2 : PRÉDICTION PAR SECTEUR ET ANNÉE
# ----------------------------------------------------
with col2:
    st.subheader("🔮 Prédiction des Offres d'Emploi")

    # S'assurer qu'on a assez de données pour entraîner un modèle
    ...
    if not secteurs_filtres:
        ...
        if st.button("Lancer la prédiction"):
            ...
            # Ajouter la ligne verticale pour l'année cible
            ax2.axvline(annee_cible, color='gray', linestyle='--')

            # Ajouter la prédiction avec un point rouge
            ax2.scatter(annee_cible, pred, color='red', label='Prédiction')

            # Personnalisation des axes
            ax2.set_xticks(range(min(data_sec['Annee']), max(data_sec['Annee']) + 1))  # Placer les années sur l'axe des X
